...
- **Admin dashboard** (protected, requires Clerk login):
  ...
  - Classifier prompt versioning (under `/admin/statistics`) — the classifier system prompt lives in `classifier_prompt_versions` (migration 010) instead of only in Python. AI advice returns surgical `prompt_edits` that are merged onto the active prompt server-side, reviewed in a diff editor, backtested against already-decided trials (`POST /api/v1/trials/insights/backtest`), then applied or rolled back
...
- **AI label display rename**: admin-facing UI now shows `Match` / `Partial Match` / `Not Suitable` instead of the underlying `confident` / `unsure` / `reject` values. The database, prompt, and API contract still use the original enum strings — display-only change in `AiClassificationCard`, `ReviewQueuePage`, `LandingPage`, and the statistics page. The map lives once in `formatAiLabel` (`utils/formatters.ts`) so the pages cannot drift apart.
...
- 113 tests passing (API, ingestion pipeline, AI services, UPDATED-trial guardrail, status filtering + facets, statistics/insights, prompt merging)
